...
!  Logical line assembled. First look for input directives
  L=1
  do while (char(L:L) .eq. space .and. char(L:L) .eq. "=" .and. L .lt. last)
    L=L+1
  end do
  if (char(L:L) .eq. "#") then
...
l=loc(item)
if (char(l:l) .eq. squote .or. char(l:l) .eq. dquote .or. char(l:l) .eq. "=") then
  m=char(l+1:end(item)-1)
else if (char(end(item):end(item)) .eq. "]") then  
  m=char(l:end(item)-1)
else if (char(l:l) .eq. "[") then  
  m=char(l+1:end(item))
else
  m=char(l:end(item))
endif
